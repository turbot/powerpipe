...
func (r *Require) validateSteampipeVersion(modName string) error {
	if steampipeVersionConstraint := r.SteampipeVersionConstraint(); steampipeVersionConstraint != nil {
		if !steampipeVersionConstraint.Check(version.PowerpipeVersion) {
			return fmt.Errorf("steampipe version %s does not satisfy %s which requires version %s", version.PowerpipeVersion.String(), modName, r.Steampipe.MinVersionString)
		}
	}
	return nil
}
