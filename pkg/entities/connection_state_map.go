...
func GetRequiredConnectionStateMap(connectionMap map[string]*Connection, currentConnectionState ConnectionStateMap) (ConnectionStateMap, map[string][]Connection, *error_helpers.ErrorAndWarnings) {
	...
	missingPluginMap := make(map[string][]Connection)
	...
	for name, connection := range connectionMap {
		...
		if connection.ImportSchema == ImportSchemaDisabled {
...
func newErrorConnectionState(connection *Connection) *ConnectionState {
