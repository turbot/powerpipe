...
func NewRuntimeDependencySubscriber(resource powerpipe2.DashboardLeafNode, parent dashboardtypes.DashboardParent, run dashboardtypes.DashboardTreeRun, executionTree *DashboardExecutionTree) *RuntimeDependencySubscriberImpl {
...
func (s *RuntimeDependencySubscriberImpl) initRuntimeDependencies(executionTree *DashboardExecutionTree) error {
	if _, ok := s.resource.(powerpipe2.RuntimeDependencyProvider); !ok {
		return nil
	}
...
func (s *RuntimeDependencySubscriberImpl) initBaseRuntimeDependencySubscriber(executionTree *DashboardExecutionTree) error {
	if base := s.resource.(modconfig.HclResource).GetBase(); base != nil {
		if _, ok := base.(powerpipe2.RuntimeDependencyProvider); ok {
			...
			s.baseDependencySubscriber = NewRuntimeDependencySubscriber(base.(powerpipe2.DashboardLeafNode), nil, s, executionTree)
...
func (s *RuntimeDependencySubscriberImpl) resolveRuntimeDependencies() error {
	rdp, ok := s.resource.(powerpipe2.RuntimeDependencyProvider)
	if !ok {
		return nil
	}
...
func (s *RuntimeDependencySubscriberImpl) findRuntimeDependencyPublisher(runtimeDependency *powerpipe2.RuntimeDependency) RuntimeDependencyPublisher {
...
func (s *RuntimeDependencySubscriberImpl) resolveSQLAndArgs() error {
	...
	queryProvider, ok := s.resource.(powerpipe2.QueryProvider)
	if !ok {
	...
	if queryProvider.RequiresExecution(queryProvider) {
		...
		args, err := powerpipe2.ResolveArgs(queryProvider, runtimeArgs)
		if err != nil {
			return err
		}
		s.Args = args
...
func (s *RuntimeDependencySubscriberImpl) populateParamDefaults(provider powerpipe2.QueryProvider) error {
...
func (s *RuntimeDependencySubscriberImpl) buildRuntimeDependencyArgs() (*powerpipe2.QueryArgs, error) {
	res := powerpipe2.NewQueryArgs()
