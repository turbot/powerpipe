...
func HomeDirectoryModfileCheck(ctx context.Context, workspacePath string) error {
	...
	// get the command and home dir
	// command := viper.Get(constants.ConfigKeyActiveCommand).(*cobra.Command)
	...
	if workspacePath == home && modFileExists {
		...
		// TODO uncomment - need that as this will be shared code for steampipe and powerpipe
		// // for interactive query - ask for confirmation to continue
		// if command.Name() == "query" && viper.GetBool(constants.ConfigKeyInteractive) {
		...
		// if task.IsBatchQueryCmd(command, viper.GetStringSlice(constants.ConfigKeyActiveCommandArgs)) && cmdconfig.Viper().GetString(constants.ArgOutput) == constants.OutputFormatTable {
