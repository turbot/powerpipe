...
@test "dashboard list includes all tags in lazy mode" {
  ...
  run bash -c "echo '$json_output' | jq -e '.[] | select(.short_name == \"dashboard_with_tags\") | .tags.service == \"test_service\"'"
  echo "Service tag check: $output"
  assert_success

  run bash -c "echo '$json_output' | jq -e '.[] | select(.short_name == \"dashboard_with_tags\") | .tags.category == \"comparison\"'"
...
@test "benchmark list includes all tags in lazy mode" {
  ...
  run bash -c "echo '$json_output' | jq -e '.[] | select(.short_name == \"benchmark_with_tags\") | .tags.service == \"test_service\"'"
  echo "Service tag check: $output"
  assert_success

  run bash -c "echo '$json_output' | jq -e '.[] | select(.short_name == \"benchmark_with_tags\") | .tags.category == \"comparison\"'"
  echo "Category tag check: $output"
  assert_success
...
@test "dashboard list includes titles in lazy mode" {
  ...
  run bash -c "echo '$json_output' | jq -e '.[] | select(.short_name == \"dashboard_with_tags\") | .title == \"Dashboard With Tags\"'"
...
@test "benchmark list includes titles in lazy mode" {
  ...
  run bash -c "echo '$json_output' | jq -e '.[] | select(.short_name == \"benchmark_with_tags\") | .title == \"Benchmark With Tags\"'"
  echo "Title check: $output"
  assert_success
...
@test "dashboard list includes descriptions in lazy mode" {
  ...
  run bash -c "echo '$json_output' | jq -e '.[] | select(.short_name == \"dashboard_with_tags\") | .description != null'"
...
@test "control list includes tags in lazy mode" {
  ...
  run bash -c "echo '$json_output' | jq -e '.[] | select(.short_name == \"control_with_tags\") | .tags.control_type == \"test\"'"
