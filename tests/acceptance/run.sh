...
export TEST_SCRIPTS_DIR=$MY_PATH/test_data/scripts
...
declare -a required_commands=("jq" "sed" "powerpipe" "rm" "mv" "cp" "mkdir" "cd" "head" "wc" "find" "basename" "dirname" "touch" "jd" "openssl" "cksum")
...
if [[ ! ${POWERPIPE_INSTALL_DIR} ]];
then
  export POWERPIPE_INSTALL_DIR="$HOME/.powerpipe"
fi

batversion=$(bats --version)
echo $batversion
echo "Running with POWERPIPE_INSTALL_DIR set to: $POWERPIPE_INSTALL_DIR"
echo "Running with binary from: $(which powerpipe)"
